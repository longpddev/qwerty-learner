    ...
    <Popover className="relative">
      {({ open }) => (
        <>
          ...
          >
            <Tooltip content="Pronunciation and phonetic symbol switching">{currentLabel}</Tooltip>
          </Popover.Button>
              ...
              <div className="shadow-upper box-border flex w-60 select-none flex-col items-center justify-center gap-4 rounded-xl bg-white p-4 drop-shadow transition duration-1000 ease-in-out dark:bg-gray-800">
                <div className="flex w-full  flex-col  items-start gap-2 py-0">
                  <span className="text-sm font-normal leading-5 text-gray-900 dark:text-white dark:text-opacity-60">
                    Switch phonetic symbol display
                  </span>
                  <div className="flex w-full flex-row items-center justify-between">
                    <Switch checked={phoneticConfig.isOpen} onChange={onChangePhoneticIsOpen} className="switch-root">
                      <span aria-hidden="true" className="switch-thumb" />
                    </Switch>
                    <span className="text-right text-xs font-normal leading-tight text-gray-600">{`Phonetic symbols have been${
                      phoneticConfig.isOpen ? 'turn on' : 'off'
                    }`}</span>
                  </div>
                </div>
                <div className="flex w-full  flex-col  items-start gap-2 py-0">
                  <span className="text-sm font-normal leading-5 text-gray-900 dark:text-white dark:text-opacity-60">
                    switch word pronunciation
                  </span>
                  <div className="flex w-full flex-row items-center justify-between">
                ...
                >
                  <div className="flex w-full  flex-col  items-start gap-2 py-0">
                    ...
                    <div className="flex w-full flex-row items-center justify-between">
                      ...
                      <span className="text-right text-xs font-normal leading-tight text-gray-600">{`The loop has been${
                        pronunciationConfig.isLoop ? 'turn on' : 'off'
                      }`}</span>
                    </div>
                  </div>
                  <div className="flex w-full  flex-col  items-start gap-2 py-0">
                  ...
                  {pronunciationConfig.isOpen && (
                    <span className="text-colo text-xs font-medium text-gray-500 dark:text-white dark:text-opacity-60">
                      Tips: Read pronunciation shortcut keys ({CTRL} + J)
                    </span>
                  )}
                </Transition>
              </div>
